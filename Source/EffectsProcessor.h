...
public:
  ...
  void updateParameters(float distDrive, float distMix, float delayTime,
                        float delayFeedback, float delayMix, float delayWidth,
                        float reverbSize, float reverbDecay, float reverbDamping,
...
private:
  ...
  // 0 = mono (both lines fed with summed input)
  // 1 = ping-pong (L feeds R delay line, R feeds L delay line)
  juce::LinearSmoothedValue<float> delayWidthParam;
