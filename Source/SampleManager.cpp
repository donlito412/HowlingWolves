...
bool SampleManager::consumeApplyHostEnvelopeDefaults() {
  const bool v = applyHostEnvelopeAfterLoad;
  applyHostEnvelopeAfterLoad = false;
  return v;
}

void SampleManager::loadSound(const juce::File &file,
                              bool applyHostEnvelopeDefaults) {
  ...
  lastLoadedKind = LoadedInstrumentKind::None;
  applyHostEnvelopeAfterLoad = applyHostEnvelopeDefaults;
  ...
  if (reader != nullptr) {
    ...
    bool isStrings = folder.equalsIgnoreCase("Strings");
    bool isHorns = folder.equalsIgnoreCase("Horns");
    bool isWoodwinds = folder.equalsIgnoreCase("Woodwinds");

    bool isOneShot = false;
    lastLoadedKind = LoadedInstrumentKind::Generic;

    if (isLead || isKey || isPad || isStrings || isHorns || isWoodwinds) {
      lastLoadedKind = LoadedInstrumentKind::Sustained;
      ...
      lastLoadedKind = LoadedInstrumentKind::Pluck;
      // Map Plucks: MIDI 36..96 (C2..C7)
      allNotes.setRange(36, 61, true);

      // Root: C3 (60) matches typical guitar/harp multis; optional RootNote in WAV
      if (reader->metadataValues.containsKey("RootNote")) {
        rootNote = reader->metadataValues["RootNote"].getIntValue();
      } else {
        rootNote = 60;
      ...
      // One-shot: natural sample decay; note-off does not choke the tail early
      // (same voice model as drums/FX — avoids "held pad" behavior on plucks).
      isOneShot = true;
    } else if (isBass) {
      lastLoadedKind = LoadedInstrumentKind::Bass;
      ...
      lastLoadedKind = LoadedInstrumentKind::Hit;
      ...
      lastLoadedKind = LoadedInstrumentKind::Texture;
      // Map Textures
      if (reader->metadataValues.containsKey("RootNote")) {
      ...
      lastLoadedKind = LoadedInstrumentKind::Sequence;
...
void SampleManager::loadDrumKit(const juce::File &kitDirectory) {
  ...
  synthEngine.clearSounds();
  lastLoadedKind = LoadedInstrumentKind::Hit;
  applyHostEnvelopeAfterLoad = true;
